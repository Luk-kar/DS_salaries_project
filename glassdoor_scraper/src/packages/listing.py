...
def extract_listingBanner(listing_soup):
    ...
    try:
        listing_bannerGroup = listing_soup.find(
            "div", class_="css-ur1szg e11nt52q0")
        ...
        if company_starRating != "NA":
            try:
                companyName = listing_bannerGroup.find(
                    "div", class_="css-16nw49e e11nt52q1").getText().replace(company_starRating, '')
            except:
                companyName = "NA"
            ...
            try:
                companyName = listing_bannerGroup.find(
                    "div", class_="css-16nw49e e11nt52q1").getText()
            except:
                companyName = "NA"

        try:
            company_offeredRole = listing_bannerGroup.find(
                "div", class_="css-17x2pwl e11nt52q6").getText()
        except:
            company_offeredRole = "NA"

        try:
            company_roleLocation = listing_bannerGroup.find(
                "div", class_="css-1v5elnn e11nt52q2").getText()
        except:
            company_roleLocation = "NA"
...
def extract_listingDesc(listing_soup):
    ...
    try:
        listing_jobDesc_raw = listing_soup.find(
